...
class ExpansionImplWithCm(ExpansionImpl):
    ...
    def _init_global(self):
        ...
        self.context.remote_cm_xml_file = os.path.join(self.tempFileDir, "cluster_cm.xml")
    ...
    def send_xml(self):
        ...
        self.ssh_tool.scpFiles(self.context.xmlFile, self.context.remote_cm_xml_file,
    ...
    def preinstall_run(self):
        ...
        cmd = "{0} -U {1} -G {2} -X {3} -L {4} " \
              "--non-interactive 2>&1".format(pre_install_path,
                                              self.user,
                                              self.group,
                                              self.context.remote_cm_xml_file,
                                              sep_env_file)
    ...
    def _set_pgxc_node_name(self):
        ...
        su_cmd = """su - {0} -c "source {1}; {2}" """.format(self.user, self.envFile, cmd)
