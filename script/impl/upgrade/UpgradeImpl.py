...
class UpgradeImpl:
    ...
    def switchDn(self, isRollback):
        ...
        if self.context.modifyLauncherParam and not isRollback:
            ...
            self.setGUCValue('enable_subscription', 'on', action_type='set', upgrade_node=True)

        if not self.context.hasTxnBuffersParam and not isRollback:
            self.context.logger.debug("need to set clog_buffers and csnlog_buffers the same value as shared_buffers when upgrade")
            self.setGUCValue('clog_buffers', self.context.sharedBuffers, action_type='set', upgrade_node=True)
            self.setGUCValue('csnlog_buffers', self.context.sharedBuffers, action_type='set', upgrade_node=True)
